...
/// system and `ps`/`pgrep`.
///
/// Each case is the automated equivalent of one of the shell runbooks
/// that live under `scripts/manual-tests/`. The runbooks are still useful
/// for catastrophic scenarios the runner can't safely script (real
/// system sleep, physical lid close) but the common cases run unattended
/// here via `scripts/run-integration.sh`.
...
/// observation window. The duty cycler alternates SIGSTOP/SIGCONT at 10
/// Hz; 90% of the time it's stopped, so a 2s poll should find state "T"
/// easily if throttling is live.
func test_16_SIGSTOPLands(_ runner: AirAssistRunner) throws {
    runner.seedExecutableRule(name: "yes", duty: 0.1)

    let proc = try runner.spawnYesTarget()
    let pid = proc.processIdentifier
    try assertGreaterThan(pid, 0)
    ...
    try assertNotNil(state, "export-state never reported pid=\(pid) throttled")
}
...
/// SIGCONT the orphaned PID via the inflight file before arming any new
/// throttle. Without the dead-man's-switch, `yes` stays stuck in state
/// "T" forever.
func test_17_CrashRecoveryResumesOrphaned(_ runner: AirAssistRunner) throws {
    runner.seedExecutableRule(name: "yes", duty: 0.1)
    let proc = try runner.spawnYesTarget()
    let pid = proc.processIdentifier
    ...
    try assertNotNil(attached, "throttler never attached — can't test recovery")
...
func test_19_ExitWatcherReleasesPromptly(_ runner: AirAssistRunner) throws {
    runner.seedExecutableRule(name: "yes", duty: 0.1)
    let proc = try runner.spawnYesTarget()
    let pid = proc.processIdentifier
    ...
    // 2s is comfortably longer than NOTE_EXIT → main-actor hop latency
    // (observed ~150ms on a warm run). Generous enough to not flake, tight
    // enough to catch a real regression where the watcher never fires.
    let released = runner.waitForState(timeout: 2) { s in
        let list = s["throttledProcesses"] as? [[String: Any]] ?? []
        return !list.contains { ($0["pid"] as? Int) == Int(pid) }
    }
    try assertNotNil(released,
...
func test_34_RuleReattachAfterRelaunch(_ runner: AirAssistRunner) throws {
    runner.seedExecutableRule(name: "yes", duty: 0.1)
    ...
    try assertNotNil(state1, "rule/engine not live in session 1")
    ...
    try assertNotNil(attached, "rule did not re-attach to yes in session 2")
}
...
/// in a non-T state.
func test_38_SIGTERMResumesInflight(_ runner: AirAssistRunner) throws {
    runner.seedExecutableRule(name: "yes", duty: 0.1)
    let proc = try runner.spawnYesTarget()
    let pid = proc.processIdentifier
